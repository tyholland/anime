...
const Admin = () => {
  ...
  const [editBench, setEditBench] = useState<boolean>(false);
  const [editDraftDate, setEditDraftDate] = useState<boolean>(false);
  const [editDraftTime, setEditDraftTime] = useState<boolean>(false);
  const [editLeague, setEditLeague] = useState<boolean>(false);
  const [leagueName, setLeagueName] = useState<string | null>(null);
  const [teamNum, setTeamNum] = useState<string | null>(null);
  const [benchNum, setBenchNum] = useState<string | null>(null);
  ...
  const [draftMeridiem, setDraftMeridiem] = useState<string | null>(null);
  const [teamNames, setTeamNames] = useState<Record<string, any> | null>(null);
  const [errorMsg, setErrorMsg] = useState<string | null>(null);
  const [league, setLeague] = useState<Record<string, any> | null>(null);
  const [missingTeams, setMissingTeams] = useState<Record<string, any>>([]);
  const [isLeagueDisabled, setIsLeagueDisabled] = useState<boolean>(true);
  const [isStarted, setIsStarted] = useState<boolean>(false);
  const [draftNotify, setDraftNotify] = useState<boolean>(false);
  ...
  const handleNumTeams = async (val: string) => {
    const payload = {
      name: leagueName,
      teams: val,
      isActive: 1,
    ...
    try {
      ...
      if (teamNames.length < parseInt(val)) {
        const remainingTeams = parseInt(val) - teamNames.length;
  ...
  const handleNumBench = async (val: string) => {
    const payload = {
      name: leagueName,
      bench: val,
      isActive: 1,
  ...
  const handleDraftTime = async () => {
    ...
    try {
      ...
      addEvent(
        'Error',
        responseError(err, 'Failed to update league draft time')
      );
    }
  };
                ...
                <Styles.AdminContainer>
                  ...
                  >
                    <Styles.AdminWrapper className="column">
                      <ol>
                        {teamNames.map((team: Record<string, any>) => {
                          return (
