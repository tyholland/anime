...
const Bracket = () => {
  ...
  const [matches, setMatches] = useState<Record<string, any> | null>(null);
  const [winWidth, setWinWidth] = useState<number>(0);
  const [modalIsOpen, setModalIsOpen] = useState<boolean>(false);
  const [bracketIsOpen, setBracketIsOpen] = useState<boolean>(false);
  const [modalMsg, setModalMsg] = useState<React.JSX.Element | string | null>(null);
  const [errorPage, setErrorPage] = useState<boolean>(false);
  const [bracketRound, setBracketRound] = useState<number | null>(null);
  const [bracketCreator, setBracketCreator] = useState<string | null>(null);
  ...
  const [roundWinner, setRoundWinner] = useState<Record<string, any> | null>(null);
  let pathname = '';
  ...
  const handleBracketDisplay = async () => {
    ...
    try {
      const { allMatches, round, creator, name } = await getBracket(bracket_id as string);

      if (round === 5) {
        const { champ } = await getTheChamp(bracket_id as string, currentUser?.token);
        setTheChamp(champ);
      }
  ...
  const handleMatchDisplay = (match: Record<string, any>) => {
  ...
  const handleVotes = async (match: Record<string, any>, team: string, payload: Record<string, any>) => {
    ...
    try {
      ...
      const updateMatch = matches.findIndex(
        (obj: Record<string, any>) => obj.matchNumber == match.matchNumber
      );
  ...
  const startRound = async () => {
    ...
    try {
      switch (bracketRound) {
      case 0:
        await startRound1(bracket_id as string, currentUser?.token);
        setBracketRound(1);
        break;
      case 1:
        await startRound2(bracket_id as string, currentUser?.token);
        setBracketRound(2);
        break;
      case 2:
        await startRound3(bracket_id as string, currentUser?.token);
        setBracketRound(3);
        break;
      case 3:
        await startRound4(bracket_id as string, currentUser?.token);
        setBracketRound(4);
        break;
      case 4:
        var { champ } = await startChampRound(bracket_id as string, currentUser?.token);
        setTheChamp(champ);
        break;
      default:
        await startRound1(bracket_id as string, currentUser?.token);
