...
const ViewBrackets = () => {
  ...
  const [bracketList, setBracketList] = useState<React.JSX.Element[] | null>(null);
  const [activeBracketList, setActiveBracketList] = useState<React.JSX.Element[] | null>(null);
  ...
  const handleAllBrackets = async () => {
    ...
    try {
      ...
      const activeBrackets = allBrackets.map((item: Record<string, any>, index: number) => {
        return (
          <div key={item.id}>
            <Button
      ...
      const userSpecific = userBrackets.map((item: Record<string, any>, index: number) => {
        return (
          <div key={item.id}>
            <Button
              btnText={item.name || `Bracket #${index + 1}`}
